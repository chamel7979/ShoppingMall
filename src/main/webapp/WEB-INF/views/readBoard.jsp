...
<table class="table table-bordered">
    <tbody>
        <!-- <form> -->
        <form action="goupdateform" method="post" onsubmit="return updatecheck()" style="display: inline;">
        <c:forEach var="list" items="${sList}" varStatus="status">
            <tr>
                <th>제목 </th>
                <td><input type="text" value="${list.title }" readonly="readonly" class="form-control"/></td>
            </tr>
            <tr>
                <th>아이디 </th>
                <td><input type="text" value="${list.custid }" readonly="readonly" class="form-control"/></td>
            </tr>
            <tr>
                <th>비밀번호 </th>
                <td><input type="password" value="${list.pw }" id="pw" readonly="readonly" class="form-control"/></td>
            </tr>
            <tr>
                <th>내용 </th>
                <td><textarea rows="20" cols="30" readonly="readonly" class="form-control">${list.content}</textarea></td>
            </tr>
           <!--  <tr>
                <th>첨부파일 </th>
            ...
            <tr>
                <td colspan="2"> 
                    <input type="button" value="글 목록" class="btn btn-primary pull-left" onclick="javascript:location.href='visit'"/>
					<c:if test="${loginid == list.custid}">
							<input type="submit" value="수정" class="btn btn-primary pull-right">
						</form>
					</c:if>
					<c:if test="${loginid == list.custid}">
						<form action="deleteBoard" method="post" onsubmit="return deletecheck()" style="display: inline;">
							<input type="hidden" value="${list.pw }" name="pw" id="pw"/>
							<input type="hidden" value="${list.boardnum }" name="boardnum" id="boardnum">
